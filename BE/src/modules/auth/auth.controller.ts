...
export const registerController = async (req: Request, res: Response) => {
  ...
  const result = await register(name, email, password).catch(err => err);
  ...
  if (!result) {
    console.error("Register error: result is null");
    return res.status(500).json({ message: "Internal server error" });
  }
...
export const loginController = async (req: Request, res: Response) => {
  ...
  if (!result) {
    console.error("Login error: result is null");
    return res.status(500).json({ message: "Internal server error" });
  }
...
export const updateProfileController = async (req: Request, res: Response) => {
  ...
  if (!updatedUser) {
    console.error("Update profile error: updated user not found");
    return res.status(500).json({ message: "Internal server error" });
  }
...
export const oauthCallbackController = async (req: Request, res: Response) => {
  ...
  if (!user && email) {
    ...
    if (!user) {
      ...
      if (!newUserId) {
        console.error("OAuth callback error: failed to create user");
  ...
  if (!token) {
    console.error("OAuth callback error: failed to create token");
    return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/login?error=server_error`);
  }
